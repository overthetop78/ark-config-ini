...
export class LevelExperienceGenerator {
  ...
  generatePlayerLevelExperienceText(): void {
    // Récupérer les données directement depuis le service
    const playerData = this.storageService.getPlayerLevelData();
    // Vérifier que les données existent
    if (!playerData) throw new Error('Les données pour les joueurs ou les dinos ne sont pas disponibles.');
  ...
  generateDinoLevelExperienceText(): void {
    // Récupérer les données directement depuis le service
    const dinoData = this.storageService.getDinoLevelData();
    // Vérifier que les données existent
    if (!dinoData) throw new Error('Les données pour les joueurs ou les dinos ne sont pas disponibles.');
